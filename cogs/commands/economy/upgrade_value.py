...
class UpgradeValueCog(Cog):
    ...
    async def upgrade_value(self, ctx: SlashContext, amount: int, freeleech: bool = False):
        ...
        if freeleech:
            confirm_embed.description = f"{ctx.author.mention}, reach the level {stats['value_upgrade'] + amount} of " \
                                        f"brightness upgrade for {fl_token * amount} freeleech token? (yes/no/y/n)"
        else:
            confirm_embed.description = f"{ctx.author.mention}, reach the level {stats['value_upgrade'] + amount} of " \
                                        f"brightness upgrade for {inflated_cost} MB? (yes/no/y/n)"
