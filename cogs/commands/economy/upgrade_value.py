...
class UpgradeValueCog(Cog):
    ...
    async def upgrade_value(self, ctx: SlashContext, amount: int, freeleech: bool = False):
        ...
        if freeleech:
            confirm_embed.description = f"{ctx.author.mention}, reach the level {stats['value_upgrade'] + amount} of brightness upgrade " \
        ...
        if freeleech:
            ...
            embed.description = f"Successfully reached brightness level {stats['brightness_upgrade']} for {fl_token * amount} " \
