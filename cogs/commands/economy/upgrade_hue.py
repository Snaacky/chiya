    ...
    async def upgrade_hue(self, ctx: SlashContext, pack: str, freeleech: bool = False):
        ...
        if (
            ...
            if not color_check:
                embed.add_field(
                    name="​",
                    value="**Condition:** Color pack must be one the following options: red, yellow, green, cyan, blue, magenta.",
                    inline=False,
                )
            if owned_check:
                embed.add_field(
                    name="​",
                    value="**Condition:** You must not already owned the color pack yet.",
                )
            if not buffer_check:
                embed.add_field(
                    name="​",
