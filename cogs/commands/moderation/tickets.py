...
class TicketCog(Cog):
    ...
    async def open(self, ctx: SlashContext, topic: str):
        ...
        permissions = {
            discord.utils.get(ctx.guild.roles, id=settings["roles"]["trial_mod"]): discord.PermissionOverwrite(read_messages=False),
            discord.utils.get(ctx.guild.roles, id=settings["roles"]["staff"]): discord.PermissionOverwrite(read_messages=True),
        ...
        if any(role.id == settings["roles"]["vip"] for role in ctx.author.roles):
            await channel.send(f"<@&{settings['roles']['staff']}>")
    ...
    async def close(self, ctx: SlashContext):
        ...
        if not ctx.channel.category_id == settings["categories"]["ticket"] or "ticket" not in ctx.channel.name:
        ...
        role_staff = discord.utils.get(ctx.guild.roles, id=settings["roles"]["staff"])
        role_trial_mod = discord.utils.get(ctx.guild.roles, id=settings["roles"]["trial_mod"])
        ...
        ticket_log = discord.utils.get(ctx.guild.channels, id=settings["channel"]["ticket_log"])
