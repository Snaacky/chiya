...
class LinePaginator(Paginator):
    ...
    def add_line(self, line: str = '', *, empty: bool = False) -> None:
        ...
        If it is still exceeded, the excess characters are stored and placed on the next pages until
            there are none remaining (by word boundary).
        The line is truncated if `scale_to_size` is still exceeded after attempting to continue
            onto the next page.
        In the case that the page already contains one or more lines and the new lines would cause
            `max_size` to be exceeded, a new page is created.
        This is done in order to make a besteffort to avoid breaking up single lines across pages,
            while keeping the total length of the page at a reasonable size.
    ...
    def _split_remaining_words(self, line: str, max_chars: int) -> typing.Tuple[str, typing.Optional[str]]:
        """
